...
export function M2mLeadQuizSection({
  ...
  return (
    <section
      id={id}
      className={cn("scroll-mt-28 border-b border-m2m-gold/15 py-16 md:py-20", className)}
      aria-labelledby={id ? `${id}-heading` : undefined}
    >
      <M2mContainer className="max-w-3xl">
        <h2
          id={id ? `${id}-heading` : undefined}
          className="m2m-section-title text-center text-m2m-cream"
        >
          {title}
        </h2>
        {description ? (
          <div className="mx-auto mt-6 max-w-2xl text-center text-sm leading-relaxed text-m2m-cream/88 sm:text-base font-sans">
            {description}
          </div>
        ) : null}

        {showEmbed ? (
          <div className="mt-10 overflow-hidden rounded-sm border border-m2m-gold/25 bg-m2m-deep/40 shadow-[0_24px_60px_rgba(0,0,0,0.28)]">
            <div className="relative aspect-[4/3] w-full min-h-[420px] sm:min-h-[480px]">
              <iframe
                src={embedSrc}
                title={title}
        ...
        {!showEmbed && children ? <div className="mt-10">{children}</div> : null}

        {!showEmbed && !children && showExternalCta ? (
          <div className="mt-10 flex justify-center">
            <Button asChild variant="m2mGold" size="lg" className="min-h-12 px-8">
        ...
        {!showEmbed && !children && !showExternalCta ? (
          <p className="mx-auto mt-10 max-w-xl rounded-sm border border-dashed border-m2m-gold/35 bg-m2m-deep/30 px-6 py-5 text-center text-sm text-m2m-cream/80 font-sans">
        ...
        {footnote ? (
          <p className="mx-auto mt-8 max-w-xl text-center text-xs leading-relaxed text-m2m-cream/70 font-sans">
            {footnote}
          </p>
        ) : null}
      </M2mContainer>

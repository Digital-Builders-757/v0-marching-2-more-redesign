...
**Implementation:**
- Form posts to GHL via native form/webhook
- Vercel page reads response status; only redirects to `/thank-you` on HTTP 200 + GHL contact ID returned
- On failure: redirect to `/submission-error` with retry CTA + email fallback `mailto:` link
- Add workflow `M2M | All | Lead Receipt Confirmation` that fires a webhook back to your monitoring (Airtable/Sheet log) confirming receipt — gives you a 2nd-channel audit trail
...
- [ ] Foreclosure form: success page only renders on confirmed GHL receipt
- [ ] Divorce form: same
- [ ] Downsizing form: same
- [ ] Failure path: error page renders + `mailto:` fallback works
- [ ] Test by temporarily breaking GHL endpoint → confirm no false success

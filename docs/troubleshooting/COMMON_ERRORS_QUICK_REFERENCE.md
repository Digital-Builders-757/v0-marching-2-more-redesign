...
| Lead API returns **503** / `config_error` / `failed_step: opportunities_create` | Missing pipeline/stage env (**any** of the four `GHL_*PIPELINE*` / `GHL_*STAGE*` vars) | Set all four pipeline + stage IDs from `.env.example`; Vercel logs: `strict_failure_pipeline_unconfigured` |
| Lead API returns **503** / “not configured” | Missing `GHL_API_KEY` / `GHL_LOCATION_ID` or custom-field env vars | Set Vercel env from `.env.example`, or use `GHL_DRY_RUN=true` on **preview/local/staging** UI-only tests |
| Lead API **503** / `config_error` on **Vercel production** with logs `dry_run_blocked_in_production` | `GHL_DRY_RUN=true` is **rejected** when `VERCEL_ENV=production` | Unset dry-run on **production**; keep dry-run for non-prod only — see [`lib/ghl/submit-lead.ts`](../../lib/ghl/submit-lead.ts) |
...
| API **`400`** / validation on **`phone`** / **`address`** for `foreclosure_intent` leads (older server) | **`foreclosure_intent`** refinements used to run only after the DOB branch, so they were skipped when `date_of_birth` was empty | Fixed in **`lib/ghl/validate.ts`** — pull latest; **`/facing-foreclosure`** submissions must include **phone** + **property address or ZIP** |

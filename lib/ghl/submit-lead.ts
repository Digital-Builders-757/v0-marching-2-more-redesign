...
function buildOperatorNote(lead: NormalizedLead): string {
  ...
  if (lead.quizScoreRange?.trim()) out.push(`- Quiz score range: ${lead.quizScoreRange.trim()}`)
  if (lead.quizMainIssue?.trim()) out.push(`- Quiz main issue: ${lead.quizMainIssue.trim()}`)
  if (lead.quizGoal?.trim()) out.push(`- Quiz goal: ${lead.quizGoal.trim()}`)
  if (lead.quizTimeline?.trim()) out.push(`- Quiz timeline: ${lead.quizTimeline.trim()}`)
  if (lead.quizPriorAttempt?.trim()) out.push(`- Quiz prior attempt: ${lead.quizPriorAttempt.trim()}`)
  if (lead.quizResult?.trim()) out.push(`- Quiz result: ${lead.quizResult.trim()}`)
  if (lead.quizSource?.trim()) out.push(`- Quiz source: ${lead.quizSource.trim()}`)
  if (lead.quizQ1BuyerType?.trim()) out.push(`- Quiz Q1 buyer type: ${lead.quizQ1BuyerType.trim()}`)
  if (lead.quizQ2Credit?.trim()) out.push(`- Quiz Q2 credit: ${lead.quizQ2Credit.trim()}`)
  if (lead.quizQ3DownPayment?.trim()) out.push(`- Quiz Q3 down payment: ${lead.quizQ3DownPayment.trim()}`)
  if (lead.quizQ4Timeline?.trim()) out.push(`- Quiz Q4 timeline: ${lead.quizQ4Timeline.trim()}`)
  if (lead.quizQ5Concern?.trim()) out.push(`- Quiz Q5 concern: ${lead.quizQ5Concern.trim()}`)
  if (lead.foreclosureIntent) out.push(`- Foreclosure form intent: ${lead.foreclosureIntent}`)

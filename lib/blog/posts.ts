import { M2M_MEDIA } from "@/lib/m2m-media"

export type BlogPost = {
  slug: string
  title: string
  excerpt: string
  coverImage: string
  author: string
  publishedAt: string
  readTime: string
  category: string
  content: string
}

// Structure-only demo content. Swap this module to Supabase later.
export const BLOG_POSTS: BlogPost[] = [
  {
    slug: "virginia-beach-homes-fly-off-market",
    title: "Why Virginia Beach Homes Fly Off the Market and How to Leverage Expert Help",
    excerpt:
      "If you have been watching the Virginia Beach real estate scene lately, you've likely noticed a clear trend: homes priced right and presented well are selling incredibly fast. Buyers are eager, offers come quickly, and sellers often find themselves surprised by how swiftly their properties move.",
    coverImage: M2M_MEDIA.heroScreenshot,
    author: "Marching 2 More Team",
    publishedAt: "2026-03-24",
    readTime: "4 min read",
    category: "Market Insights",
    content: `## The Hampton Roads Market in 2026

If you have been watching the Virginia Beach real estate scene lately, you've likely noticed a clear trend: homes priced right and presented well are selling incredibly fast.

### Why Homes Are Moving So Quickly

- **Strong military presence** - Consistent demand from PCS families
- **Growing job market** - Expansion in shipbuilding, healthcare, and tech
- **Coastal lifestyle appeal** - Buyers want access to beaches and outdoor activities
- **Limited inventory** - More buyers than available homes

### How Expert Help Makes the Difference

Working with a team that understands the local market dynamics gives you a significant advantage. We help you:

- Price strategically based on real-time market data
- Present your home to attract qualified buyers
- Navigate multiple offers with confidence
- Close smoothly and on your timeline

> Ready to make your move? Let's talk about your goals.`,
  },
  {
    slug: "costly-mistakes-selling-without-realtor",
    title: "The Costly Mistakes of Selling Your Hampton Roads Home Without a Realtor",
    excerpt:
      "Selling your home in Hampton Roads on your own might seem like a smart way to save money. The idea of skipping the real estate agent and avoiding commission fees is tempting. But many homeowners who choose the for sale by owner Virginia route soon find out that it can cost them more than they expected.",
    coverImage: M2M_MEDIA.sellHeroStill,
    author: "Marching 2 More Team",
    publishedAt: "2026-03-10",
    readTime: "3 min read",
    category: "Selling Tips",
    content: `## The Hidden Costs of FSBO

Selling your home in Hampton Roads on your own might seem like a smart way to save money. But many homeowners who choose the for-sale-by-owner route discover it can cost them more than expected.

### Common FSBO Mistakes

1. **Pricing incorrectly** - Without access to MLS data and market expertise, many FSBO sellers either underprice (leaving money on the table) or overprice (causing the home to sit)

2. **Limited marketing reach** - Professional agents have networks, MLS access, and marketing systems that reach more qualified buyers

3. **Negotiation pitfalls** - Experienced buyers and their agents may have an advantage in negotiations

4. **Legal and paperwork risks** - Real estate contracts are complex, and mistakes can be costly

### The Numbers Tell the Story

Studies consistently show that homes sold with professional representation sell for more money, even after accounting for commission costs.

> Thinking about selling? Let us show you what your home is really worth.`,
  },
  {
    slug: "selling-coastal-properties-hampton-roads",
    title: "Selling Coastal Properties in Hampton Roads: Strategies to Elevate Your Waterfront Listing in 2026",
    excerpt:
      "Selling a home near the water in Hampton Roads requires more than just listing it on the market. Coastal properties offer unique lifestyle benefits and challenges that demand a specialized approach. Whether your home is in Ocean Lakes, Fox Hill, Willoughby Spit, or Knotts Island, standing out in 2026 means showcasing both the value and the emotional appeal of waterfront living.",
    coverImage: M2M_MEDIA.familyBackyard,
    author: "Marching 2 More Team",
    publishedAt: "2026-02-24",
    readTime: "3 min read",
    category: "Coastal Living",
    content: `## Waterfront Living in Hampton Roads

Selling a home near the water in Hampton Roads requires more than just listing it on the market. Coastal properties offer unique lifestyle benefits and challenges that demand a specialized approach.

### What Makes Coastal Properties Special

- **Lifestyle appeal** - Access to boating, fishing, and beach activities
- **Views and natural light** - Premium features buyers will pay for
- **Unique considerations** - Flood zones, insurance, and maintenance

### Marketing Strategies That Work

1. **Professional photography** - Capture the water views at golden hour
2. **Drone footage** - Show the property's relationship to the water
3. **Highlight outdoor spaces** - Decks, docks, and waterfront access
4. **Tell the lifestyle story** - Help buyers envision their life there

### Navigating Coastal Challenges

We help sellers address buyer concerns proactively:
- Flood insurance information
- Elevation certificates
- Recent maintenance and updates
- Storm preparedness features

> Have a waterfront property to sell? We specialize in coastal listings.`,
  },
  {
    slug: "role-of-real-estate-professional",
    title: "Unveiling the Role of a Real Estate Professional",
    excerpt:
      "Buying or selling a home is a big deal. It's more than just a transaction - it's about finding a place where memories will be made or moving on to a new chapter. For military families in Virginia, this process can be even more complex due to frequent relocations and unique housing needs.",
    coverImage: M2M_MEDIA.teamCtaBackdrop,
    author: "Donavan McFadden",
    publishedAt: "2026-02-24",
    readTime: "4 min read",
    category: "Buyer Tips",
    content: `## More Than a Transaction

Buying or selling a home is more than just a transaction - it's about finding a place where memories will be made or moving on to a new chapter.

### What a Real Estate Professional Actually Does

**For Buyers:**
- Understand your needs and timeline
- Search for properties that match your criteria
- Arrange showings and provide market insights
- Negotiate on your behalf
- Guide you through inspections, financing, and closing

**For Sellers:**
- Evaluate your home's market value
- Create a marketing strategy
- Manage showings and feedback
- Negotiate offers
- Coordinate the closing process

### The Military Family Difference

For military families in Virginia, the process can be more complex due to:
- PCS timelines and deadlines
- VA loan requirements
- Distance buying and selling
- BAH considerations

We understand these challenges because we've lived them.

> Ready to work with someone who gets it? Let's connect.`,
  },
  {
    slug: "va-loan-guide-military-families",
    title: "The Complete VA Loan Guide for Military Families in Hampton Roads",
    excerpt:
      "VA loans are one of the best benefits available to military service members and veterans. With no down payment required and competitive interest rates, a VA loan can make homeownership a reality for those who have served our country. This comprehensive guide covers everything you need to know.",
    coverImage: M2M_MEDIA.buyHeroStill,
    author: "Roger Lee",
    publishedAt: "2026-02-15",
    readTime: "5 min read",
    category: "VA Loans",
    content: `## Understanding Your VA Loan Benefit

VA loans are one of the best benefits available to military service members and veterans. With no down payment required and competitive interest rates, a VA loan can make homeownership a reality.

### Key VA Loan Benefits

- **No down payment** - Purchase with 0% down in most cases
- **No PMI** - Save hundreds per month compared to conventional loans
- **Competitive rates** - Often lower than conventional mortgages
- **Flexible credit requirements** - More forgiving than other loan types
- **Reusable** - Use your benefit multiple times

### Eligibility Requirements

You may be eligible if you:
- Served 90+ days during wartime
- Served 181+ days during peacetime
- Served 6+ years in the National Guard or Reserves
- Are the surviving spouse of a veteran who died in service

### The VA Loan Process

1. Obtain your Certificate of Eligibility (COE)
2. Get pre-approved with a VA-approved lender
3. Find your home with a VA-savvy agent
4. Make an offer and complete the VA appraisal
5. Close on your new home

### Common VA Loan Myths

- *Myth:* VA loans take longer to close
- *Myth:* Sellers won't accept VA offers
- *Myth:* You can only use VA once

> Questions about VA loans? We help military families navigate this every day.`,
  },
  {
    slug: "pcs-relocation-checklist",
    title: "PCS Relocation Checklist: Your Complete Guide to Moving to Hampton Roads",
    excerpt:
      "Receiving PCS orders can be both exciting and overwhelming. Moving to a new duty station involves countless details and decisions. This comprehensive checklist will help military families navigate their relocation to the Hampton Roads area with confidence and ease.",
    coverImage: M2M_MEDIA.teamPhotoWide,
    author: "Kristin Allen",
    publishedAt: "2026-02-10",
    readTime: "6 min read",
    category: "PCS Relocation",
    content: `## Your PCS Move to Hampton Roads

Receiving PCS orders can be both exciting and overwhelming. This comprehensive checklist will help you navigate your relocation to the Hampton Roads area.

### 6-8 Weeks Before Your Move

- [ ] Review your orders and report date
- [ ] Contact a local real estate agent
- [ ] Research neighborhoods near your duty station
- [ ] Start the pre-approval process if buying
- [ ] Schedule a virtual home tour or visit

### 4-6 Weeks Before Your Move

- [ ] Coordinate with TMO for household goods
- [ ] Gather important documents
- [ ] Research schools if you have children
- [ ] Set up temporary lodging if needed
- [ ] Begin packing non-essentials

### 2-4 Weeks Before Your Move

- [ ] Confirm moving dates
- [ ] Transfer utilities and update addresses
- [ ] Finalize housing arrangements
- [ ] Prepare a travel kit for the move

### Hampton Roads Duty Stations

- **Naval Station Norfolk** - World's largest naval station
- **Joint Base Langley-Eustis** - Air Force and Army
- **NAS Oceana** - Master Jet Base
- **Norfolk Naval Shipyard** - Portsmouth
- **Dam Neck** - Virginia Beach

### Neighborhoods to Consider

Each area offers different benefits depending on your duty station, family size, and lifestyle preferences. We can help you find the right fit.

> PCS orders to Hampton Roads? Let's start planning your move.`,
  },
  {
    slug: "hampton-roads-home-buying-guide",
    title: "A First-Time Home Buying Guide for Hampton Roads",
    excerpt:
      "A practical walkthrough for military families and first-time buyers: budgeting, VA loans, timelines, and what to expect.",
    coverImage:
      "https://static.wixstatic.com/media/63ece0_4d26d16a6b6f431c9ade77e6af8af089~mv2.jpg/v1/fill/w_1903,h_813,al_c,q_85,usm_0.66_1.00_0.01,enc_avif,quality_auto/63ece0_4d26d16a6b6f431c9ade77e6af8af089~mv2.jpg",
    author: "Marching 2 More",
    publishedAt: "2026-04-01",
    readTime: "6 min",
    category: "Buy",
    content: `## What to Expect

Buying a home doesn't have to be overwhelming. Here's the process we recommend:

- Clarify budget + financing
- Get a strong pre-approval
- Identify must-haves vs nice-to-haves
- Tour strategically
- Negotiate confidently

## Our Approach

We'll help you make clean decisions, avoid surprises, and keep the experience calm and organized.

> Demo note: this is placeholder content. We'll replace posts with Supabase-backed content later.`,
  },
  {
    slug: "va-loan-basics",
    title: "VA Loan Basics: What Matters and What Doesn't",
    excerpt:
      "A clear explanation of eligibility, funding fees, appraisal timelines, and common misconceptions about VA loans.",
    coverImage:
      "https://static.wixstatic.com/media/63ece0_85f19a3d9c0648609ff59e4b0a4c9578~mv2.jpg/v1/fill/w_1903,h_812,al_c,q_85,usm_0.66_1.00_0.01,enc_avif,quality_auto/63ece0_85f19a3d9c0648609ff59e4b0a4c9578~mv2.jpg",
    author: "Marching 2 More",
    publishedAt: "2026-03-21",
    readTime: "5 min",
    category: "Finance",
    content: `## The Short Version

VA loans can be an incredible advantage. The key is making sure your timeline and paperwork are tight.

### Common Myths

- *Myth:* VA loans always take longer
- *Myth:* Sellers won't accept VA

We'll help you present a clean offer and keep everything moving.`,
  },
  {
    slug: "pre-listing-checklist",
    title: "Pre-Listing Checklist: Maximize Value Before You List",
    excerpt:
      "Small upgrades + smart preparation can change your final sale price. Here's a simple checklist that works.",
    coverImage:
      "https://static.wixstatic.com/media/63ece0_0f4f064e9f5d4f3492a21f78817e3539~mv2.jpg/v1/fill/w_1903,h_812,al_c,q_85,usm_0.66_1.00_0.01,enc_avif,quality_auto/63ece0_0f4f064e9f5d4f3492a21f78817e3539~mv2.jpg",
    author: "Marching 2 More",
    publishedAt: "2026-03-10",
    readTime: "4 min",
    category: "Sell",
    content: `## The Goal

We want buyers to feel certainty the moment they walk in.

- Declutter
- Deep clean
- Fix the small obvious things
- Make curb appeal effortless

If you want the full guide, grab the Pre-Listing Checklist page.`,
  },
]

export function getPostBySlug(slug: string): BlogPost | undefined {
  return BLOG_POSTS.find((p) => p.slug === slug)
}
